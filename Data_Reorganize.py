...
def data_reorganize():
    ...
    for index1 in range(len(data_all)):
        for index2 in range(len(df_all[index1])):
            ...
            evaluation_score = df_all[index1].loc[index2, 'product_avg_evaluation_score']
            product_type = index1
            data_all[index1].append([product_id, avg_rating, evaluation_score, product_type])

        output_df_all.append(
            pd.DataFrame(data_all[index1], columns=['product_id', 'avg_rating', 'evaluation_score', 'product_type']))
